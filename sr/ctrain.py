...
from sklearn.metrics import roc_auc_score, average_precision_score, precision_recall_curve, roc_curve, f1_score, precision_score, recall_score, confusion_matrix
...
lr_steps = [("scaler", StandardScaler(with_mean=False)), ("clf", LogisticRegression(max_iter=200, class_weight="balanced"))]
rf_clf = RandomForestClassifier(n_estimators=300, class_weight="balanced", random_state=42, n_jobs=-1)
xgb = XGBClassifier(
...
rf_pipe = ImbPipeline([("smote", SMOTE(random_state=42)), ("clf", rf_clf)])
xgb_pipe = ImbPipeline([("smote", SMOTE(random_state=42)), ("clf", xgb)])
else:
lr_pipe = Pipeline(lr_steps)
rf_pipe = Pipeline([("clf", rf_clf)])
...
plt.figure(); plt.plot(fpr, tpr); plt.plot([0,1],[0,1],"--",color="gray"); plt.xlabel("FPR"); plt.ylabel("TPR"); plt.title("ROC"); plt.tight_layout(); plt.savefig(os.path.join(out_dir,"roc.png"), dpi=200); plt.close()
plt.figure(); plt.plot(recall, precision); plt.xlabel("Recall"); plt.ylabel("Precision"); plt.title("PR"); plt.tight_layout(); plt.savefig(os.path.join(out_dir,"pr.png"), dpi=200); plt.close()

def plot_confusion(cm, title, out_path):
plt.figure(); sns.heatmap(cm, annot=True, fmt="d", cmap="Blues"); plt.xlabel("Predicted"); plt.ylabel("Actual"); plt.title(title); plt.tight_layout(); plt.savefig(out_path, dpi=200); plt.close()
...
plt.figure(figsize=(6,4)); sns.barplot(x=imp[idx], y=np.array(feature_names)[idx], orient="h")
plt.title("Top Feature Importances"); plt.tight_layout(); plt.savefig(out_path, dpi=200); plt.close()
...
neg = int((y_train==0).sum()); pos = int((y_train==1).sum()); ratio = neg/max(1,pos)
...
grid = GridSearchCV(pipe, param_grid=param_grid, scoring="average_precision", cv=skf, n_jobs=-1, verbose=1)
...
thresholds = np.r_[0.5, thresholds]
...
inner = best.named_steps["clf"] if hasattr(best, "named_steps") else best
plot_importances(inner, feature_names, "results/importances.png")
